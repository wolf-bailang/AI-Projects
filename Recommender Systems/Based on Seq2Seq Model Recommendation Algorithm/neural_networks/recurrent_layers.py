...
def get_recurrent_layers():
	return RecurrentLayers(layer_type="LSTM", layers=[50], bidirectional=True, embedding_size=1)
...
class RecurrentLayers(object):
	...
	def set_name(self):
		...
		if self.bidirectional:
			self.name += "b"+self.layer_type+"_"  #bLSTM_
		elif self.layer_type != "LSTM":
			self.name += self.layer_type+"_"	#LSTM_
		
		self.name += "gc"+str(self.grad_clip)+"_"	#bLSTM_gc100_
		if self.embedding_size > 0:
			self.name += "e"+str(self.embedding_size)	#bLSTM_gc100_e1
		self.name += "h"+('-'.join(map(str,self.layers)))	#bLSTM_gc100__e1-h50
