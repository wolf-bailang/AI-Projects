...
def saveResult(save_path, npyfile, flag_multi_class=False, num_class=2):
    for i, item in enumerate(npyfile):
        ...
        # img = img / 255
        img = trans.resize(img, (1200, 500))
        io.imsave(os.path.join(save_path), img)

        # img[marks == -1] = [255, 0, 0]


# ##########################################################################################################################################
def Hist1(image_path, Hist_path, as_gray=True):
    img = io.imread(os.path.join(image_path), as_gray=as_gray)
    # gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    gray = img  # / 255
    eq = cv2.equalizeHist(gray)
    # eq = eq * 255
    cv2.imwrite(os.path.join(Hist_path), np.hstack([eq]))
# ##########################################################################################################################################
