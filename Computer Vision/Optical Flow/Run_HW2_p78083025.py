...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def Preprocessing_click(self):
        ...
        if cap.isOpened():  # VideoCaputre对象是否成功打开
            ...
            while ret:
                ...
                im_with_keypoints = cv2.line(im_with_keypoints, (temp[0], temp[1]-5), (temp[0], temp[1]+5), (0, 0, 255), 1)
                im_with_keypoints = cv2.rectangle(im_with_keypoints, (temp[0]-5, temp[1]-5), (temp[0]+5, temp[1]+5), (0, 0, 255), 1)
                for j in range(1, k):
                    # print('i= ' + str(i))
                    im_with_keypoints = cv2.line(im_with_keypoints, (temp[j*2]-5, temp[j*2+1]), (temp[j*2]+5, temp[j*2+1]), (0, 0, 255), 1)
                    im_with_keypoints = cv2.line(im_with_keypoints, (temp[j*2], temp[j*2+1]-5), (temp[j*2], temp[j*2+1]+5), (0, 0, 255), 1)
                    im_with_keypoints = cv2.rectangle(im_with_keypoints, (temp[j*2]-5, temp[j*2+1]-5), (temp[j*2]+5, temp[j*2+1]+5), (0, 0, 255), 1)
                '''
    ...
    def Perspective_Transform_click(self):
        ...
        for j in range(0, i):
            ...
            retval, mask = cv2.findHomography(np.array(pts_src, dtype=np.float32), np.array(pts_dst, dtype=np.float32), cv2.RANSAC, 5.0)
            ...
            dst = cv2.warpPerspective(src=img_src, M=retval, dsize=(width, height), dst=imgorg, flags=cv2.WARP_FILL_OUTLIERS, borderMode=cv2.BORDER_TRANSPARENT, borderValue=None)   #, frameflags=
    ...
    def Image_Reconstruction_click(self):
        self.get_current_path()
        for i in range(1, 35):
            img = cv2.imread(self.current_path + '/Q4_Image/' + str(i) + '.jpg')
            # Load the image
        ...
        for i in range(1, 69):
            if i <= 17:
                ax = plt.subplot(4, 17, i)
                im = plt.imread(self.current_path + '/Q4_Image/result/' + str(i) + '.jpg')
                ax.imshow(im)
            elif i <= 34:
                ax = plt.subplot(4, 17, i)
                im = plt.imread(self.current_path + '/Q4_Image/result/' + str(i) + '.jpg')
                ...
                ax = plt.subplot(4, 17, i)
                im = plt.imread(self.current_path + '/Q4_Image/result/' + str(i - 17) + '.jpg')
                ax.imshow(im)
            else:
                ax = plt.subplot(4, 17, i)
                im = plt.imread(self.current_path + '/Q4_Image/result/' + str(i - 17) + '.jpg')
        ...
        '''
        '''
        fig, axs = plt.subplots(nrows=4, ncols=17, figsize=(10, 10))
